/*
Consider a HashSet as a HashMap where we just care about the keys ( HashSet<T> 
...
elements. That's the contract that any set collection fulfills. HashSet is just 
one implementation. (see also: BTreeSet)
...
2) difference: get all the elements that are in the first set but not the second.
3) intersection: get all the elements that are only in both sets.
4) symmetric_difference: get all the elements that are in one set or the other, 
but not both.
