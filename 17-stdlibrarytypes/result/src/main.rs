...
sometimes it is important to express why an operation failed. To do this we have 
the Result enum.

The Result<T, E> enum has two variants:

1) Ok(value) which indicates that the operation succeeded, and wraps the value 
returned by the operation. (value has type T)
2) Err(why), which indicates that the operation failed, and wraps why, which 
